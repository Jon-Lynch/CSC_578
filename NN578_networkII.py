...
class CrossEntropyCost(object):
    ...
    def derivative(a, y):
        """Return the first derivative of the function."""
        return (a - y) / np.nan_to_num(a * (1 - a))
...
''' Softmax is rarely used for hidden layers (act_hidden parameter) and
options for output layer activations are Sigmoid, Tanh, and Softmax. If
Tanh is selected for activation function at output layer, cost function
must be QuadraticCost. '''
...
class Softmax(object):
    ...
    def fn(z):
        ...
        return np.exp(z) / np.nan_to_num(np.sum(np.exp(z)))
...
class Network(object):
    ...
    def __init__(self, sizes, cost=CrossEntropyCost, act_hidden=Sigmoid,
        ...
        self.sizes = sizes
        self.hidden_layers = self.sizes[1:-1]  # obtain just the hidden layers
    ...
    def set_parameters(self, cost=QuadraticCost, act_hidden=Sigmoid,
        ...
        # ensure cost function is Quadratic if Tanh activation used for output layer:
        if self.act_output == Tanh:
            if self.cost != QuadraticCost:
                print(
                    "Tanh only accepts 'QuadraticCost' cost function. Changing to QuadraticCost.")
    ...
    def feedforward(self, a):
        ...
        for b, w in zip(self.biases[:-1], self.weights[:-1]):
            ...
            # activation at the last hidden layer:
        ...
        a = (self.act_output).fn(np.dot(w, a) + b)  # output layer activations
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        if evaluation_data:
            n_data = len(evaluation_data)  # lenght of test set
        n = len(training_data)
        ...
        for j in range(epochs):
            ...
            # obtain dropout masks for hidden layers if dropout percent specified:
            if self.dropoutpercent != 0.0:
                self.masks = []
                for i in range(len(self.hidden_layers)):
                    mask = np.random.binomial(1, (1 - self.dropoutpercent), size=self.hidden_layers[i]) / (1 - self.dropoutpercent)
            ...
            if monitor_training_cost:
                # nt: for cost, always NO convert (default) for training
                cost = self.total_cost(training_data, lmbda)
                training_cost.append(cost)
                print("Cost on training data: {}".format(cost))
            if monitor_training_accuracy:
                # nt: for accuracy, always _DO_ convert (argmax) for training
            ...
            if monitor_evaluation_cost:
                ...
                if no_convert:
                    # nt: if test/val data is already vectorized for y
            ...
            if monitor_evaluation_accuracy:
                ...
                print("Accuracy on evaluation data: {} / {}".format(
                    ...
                    accuracy, n_data))
            print()
        # reordered to outout training results 1st, then test results:
    ...
    def backprop(self, x, y):
        ...
        activation = (self.act_output).fn(z)  # activation for output layer
        activations.append(activation)

        # apply dropout masks to hidden layer activations if dropout specified:
        if self.dropoutpercent != 0.0:
            for i in range(len(self.hidden_layers)):
                activations[i + 1] *= self.masks[i].reshape(len(activations[i + 1]), 1)
                # activations[1] *= self.masks[0].reshape(20, 1)
                # activations[2] *= self.masks[1].reshape(7, 1)
        ...
        for l in range(2, self.num_layers):
            ...
            # apply dropout masks to deltas of hidden layers if dropout specified:
            if self.dropoutpercent != 0.0:
                delta *= self.masks[-l + 1].reshape(len(activations[-l]), 1)
                # if delta.shape == (7,1):
                # delta *= self.masks[1].reshape(7,1)
                # elif delta.shape == (20,1):
                # delta *= self.masks[0].reshape(20,1)
    ...
    def total_cost(self, data, lmbda, convert=False):
        ...
        # L1 regularization additional cost:
        if self.regularization == 'L1':
            cost += (lmbda / len(data)) * sum(abs(np.linalg.norm(w)) for w in self.weights)
        # L2 regularization additional cost:
        else:
